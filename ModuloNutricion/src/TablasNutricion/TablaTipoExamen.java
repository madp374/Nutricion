 ...
 */
@WebServlet("/TablaTipoExamen")
public class TablaTipoExamen extends HttpServlet {
	private static final long serialVersionUID = 1L;
	static Connection conn = null;
    /**
     * @see HttpServlet#HttpServlet()
     */
    public TablaTipoExamen() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(action.equalsIgnoreCase("eliminardato")){
			...
			String IdBorrar=request.getParameter("idTIPO_EXAMEN");
			String sql = "delete from TIPO_EXAMEN where idTIPO_EXAMEN="+IdBorrar+"; ";
			...
			String query2="insert into TIPO_EXAMEN(nombre) values('"+auxa+"')";
			String result="";
			...
			try {
				...
				String sql = "UPDATE TIPO_EXAMEN SET nombre='"+auxa+"'  WHERE idTIPO_EXAMEN="+ID+"; ";
			...
			String sql = "SELECT * FROM TIPO_EXAMEN " + where + " " + sort + " " + limit;
			...
			try {
				...
				total = countRec("idTIPO_EXAMEN", "TIPO_EXAMEN $where");
	...
	private String getJson(ResultSet rs, int total, int page) throws Exception {
		...
		Conexion consulta = new Conexion();
		while (rs.next()) {
			String idTIPO_EXAMEN = rs.getString(1);
			String nombre = consulta.Codificar(rs.getString(2));
			
			
			buffer+="<row id='"+idTIPO_EXAMEN+"'>";
			buffer+="<cell><![CDATA[<input type='radio' class='menu_radio' name='id_radio' onclick='DatosSeleccionados("+idTIPO_EXAMEN+",\""+nombre+"\")' value='"+idTIPO_EXAMEN+"' />]]></cell>";
			buffer+="<cell><![CDATA["+idTIPO_EXAMEN+"]]></cell>";
