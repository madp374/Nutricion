...
/**
 * Servlet implementation class TablaConsultaExterna
 */
@WebServlet("/TablaConsultaExterna")
public class TablaConsultaExterna extends HttpServlet {
	private static final long serialVersionUID = 1L;
	static Connection conn = null;
    /**
     * @see HttpServlet#HttpServlet()
     */
    public TablaConsultaExterna() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(action.equalsIgnoreCase("eliminardato")){
			...
			String IdBorrar=request.getParameter("idCONSULTA_EXTERNA");
			String sql = "delete from CONSULTA_EXTERNA where idCONSULTA_EXTERNA="+IdBorrar+"; ";
			...
			String sql = "SELECT * FROM CONSULTA_EXTERNA " + where + " " + sort + " " + limit;
			...
			try {
				...
				total = countRec("idCONSULTA_EXTERNA", "CONSULTA_EXTERNA $where");
	...
	private String getJson(ResultSet rs, int total, int page) throws Exception {
		String buffer = "";
		while (rs.next()) {
			String idCONSULTA_EXTERNA = rs.getString(1);
			String fecha = rs.getString(2);
			String Paciente_idPaciente = rs.getString(3);
			
			
			buffer+="<row id='"+idCONSULTA_EXTERNA+"'>";
			buffer+="<cell><![CDATA[<input type='radio' class='menu_radio' name='id_radio' onclick='DatosSeleccionados("+idCONSULTA_EXTERNA+")' value='"+idCONSULTA_EXTERNA+"' />]]></cell>";
			buffer+="<cell><![CDATA["+idCONSULTA_EXTERNA+"]]></cell>";
